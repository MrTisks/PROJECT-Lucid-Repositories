Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/08e63796899f9fa49ba6a4aab15c1858"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 RandomColorFromPoint(float2 rando, PoiMods poiMods)
			{
				...
				fixed saturation = lerp(float4(0.9062848,1,0,1).x, float4(0.9062848,1,0,1).y, rando.x);
				fixed value = lerp(float4(0.4903885,1,0,1).x, float4(0.4903885,1,0,1).y, rando.y);
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				{
					float2 st = (poiMesh.uv[(0.0 /*_GlitterUV*/)] + float4(0.001,0.001,0,0).xy * _Time.x) * (253.57 /*_GlitterFrequency*/);
					...
					float size = (0.168 /*_GlitterSize*/);
					
					if ((1.0 /*_GlitterRandomSize*/))
					{
						size = lerp(float4(0.2402958,0.7042514,0,1).x, float4(0.2402958,0.7042514,0,1).y, randomFromPoint);
					}
					#ifdef POI_AUDIOLINK
					if ((0.0 /*_GlitterALEnabled*/))
					{
					...
					switch((0.0 /*_GlitterShape*/))
					{
						...
						if ((0.0 /*_GlitterRandomRotation*/) == 1 || (0.1 /*_GlitterTextureRotation*/) != 0 || float4(-20,20,0,0).x != 0 || float4(-20,20,0,0).y != 0 || glitterRotationTimeOffset != 0)
						{
							float2 center = float2(0, 0);
							float2 glitterRandomRotationSpeed = 0;
							float randomBoy = 0;
							
							if ((0.0 /*_GlitterRandomRotation*/) || float4(-20,20,0,0).x != 0 || float4(-20,20,0,0).y != 0)
							{
								randomBoy = random(m_point * 200);
								glitterRandomRotationSpeed = lerp(float4(-20,20,0,0).x, float4(-20,20,0,0).y, randomBoy);
							}
							if (glitterRandomRotationSpeed.x + glitterRandomRotationSpeed.y + (0.1 /*_GlitterTextureRotation*/) == 0 && glitterRotationTimeOffset != 0)
							{
								glitterRandomRotationSpeed = 1;
							}
							float theta = radians((randomBoy + (_Time.x + glitterRotationTimeOffset) * ((0.1 /*_GlitterTextureRotation*/) + glitterRandomRotationSpeed)) * 360);
							float cs = cos(theta);
							float sn = sin(theta);
					...
					half3 glitterColor = poiThemeColor(poiMods, float4(0,1,0.659116,1).rgb, (0.0 /*_GlitterColorThemeIndex*/));
					float3 norm = lerp(poiMesh.normals[0], poiMesh.normals[1], (1.0 /*_GlitterUseNormals*/));
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						if ((2.93 /*_GlitterSpeed*/) + glitterSpeedOffset > 0)
						{
							randomRotation = randomFloat3WiggleRange(randoPoint, (59.0 /*_GlitterAngleRange*/), (2.93 /*_GlitterSpeed*/), glitterSpeedOffset);
						}
						else
						{
							randomRotation = randomFloat3Range(randoPoint, (59.0 /*_GlitterAngleRange*/));
						}
						float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.401 /*_GlitterBias*/)), poiCam.viewDir)), (295.0 /*_GlitterContrast*/)), 0);
						finalGlitter *= glitterAlpha;
						break;
						case 1:
						float randomOffset = random(randoPoint);
						float brightness = (sin((_Time.x * 10 + randomOffset +glitterSpeedOffset) * (2.93 /*_GlitterSpeed*/)) * .5 + .5);
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(specAngle, (295.0 /*_GlitterContrast*/)), 0);
					...
					#if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
					glitterColor *= POI2D_SAMPLER_PAN(_GlitterColorMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_GlitterColorMapUV*/)], float4(1,1,0,0)), float4(1,1,0,0)).rgb;
					#endif
					float2 uv = remapClamped(-size, size, dank, 0, 1);
					
					if ((0.0 /*_GlitterRandomRotation*/) == 1 || (0.1 /*_GlitterTextureRotation*/) != 0 || float4(-20,20,0,0).x != 0 || float4(-20,20,0,0).y && !(0.0 /*_GlitterShape*/) || glitterRotationTimeOffset != 0)
					{
					...
					#if defined(PROP_GLITTERMASK) || !defined(OPTIMIZER_ENABLED)
					float glitterMask = POI2D_SAMPLER_PAN(_GlitterMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_GlitterMaskUV*/)], float4(1,1,0,0)), float4(0,0,0,0))[(3.0 /*_GlitterMaskChannel*/)];
					#else
					float glitterMask = 1;
					#endif
					#ifdef POI_AUDIOLINK
					...
					glitterMask *= lerp(1, poiLight.rampedLightMap, (0.435 /*_GlitterHideInShadow*/));
					glitterMask *= lerp(1, poiLight.directLuminance, (1.0 /*_GlitterScaleWithLighting*/));
					glitterMask *= float4(0,1,0.659116,1).a;
					if ((0.0 /*_GlitterMaskGlobalMask*/) > 0)
					{
						glitterMask = maskBlend(glitterMask, poiMods.globalMask[(0.0 /*_GlitterMaskGlobalMask*/) - 1], (1.0 /*_GlitterMaskGlobalMaskBlendType*/));
					}
					...
					if ((1.0 /*_GlitterBlendType*/) == 1)
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * ((40.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, (((40.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * ((40.0 /*_GlitterBrightness*/) + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PBRMaps = POI2D_SAMPLER_PAN_STOCHASTIC(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[(0.0 /*_MochieMetallicMapsUV*/)], float4(5,5,0,0)), float4(0,0,0,0), (0.0 /*_MochieMetallicMapsStochastic*/));
				...
				if ((0.0 /*_PBRSplitMaskSample*/))
				{
					...
					assignValueToVectorFromIndex(PBRMaps, (3.0 /*_MochieMetallicMapsReflectionMaskChannel*/), PBRSplitMask[(3.0 /*_MochieMetallicMapsReflectionMaskChannel*/)]);
				...
				if ((3.0 /*_MochieMetallicMapsRoughnessChannel*/) < 4)
				{
					smoothness *= PBRMaps[(3.0 /*_MochieMetallicMapsRoughnessChannel*/)];
					smoothness2 *= PBRMaps[(3.0 /*_MochieMetallicMapsRoughnessChannel*/)];
				}
				if ((3.0 /*_MochieMetallicMapsReflectionMaskChannel*/) < 4)
				{
					reflectionMask *= PBRMaps[(3.0 /*_MochieMetallicMapsReflectionMaskChannel*/)];
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[(0.0 /*_MainTexUV*/)].xy, float4(5,5,0,0));
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.2228772,0.2228772,0.2228772,1).rgb, (0.0 /*_ColorThemeIndex*/));
				poiFragData.alpha = mainTexture.a * float4(0.2228772,0.2228772,0.2228772,1).a;
				#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
